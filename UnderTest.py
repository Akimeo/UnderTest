...
class MakeTest(QMainWindow, MT):
    ...
    def main(self):
        try:
            ...
            self.act_add_question.triggered.connect(lambda:
                                                    self.new_tab(
                                                        menuCall=True
                                                        )
                                                    )
            self.act_del_question.triggered.connect(self.close_tab)
            self.act_add_ans.triggered.connect(lambda:
                                               self.add_line(
                                                   self.tab.currentIndex())
                                               )
            ...
            self.info[0]['var_btn'].clicked.connect(lambda:
                                                    self.add_line(
                                                        self.tab.currentIndex()
                                                    ))
    ...
    def new_tab(self, smth=False, menuCall=False):
        try:
            ...
            self.info[self.i]['var_btn'].clicked.connect(
                lambda: self.add_line(self.tab.currentIndex()))
    ...
    def add_line(self, pn):
        ...
        try:
            ...
            if lt:
                ...
                self.info[pn]['del_btns'].append(
                    self.info[pn]['ans_lines'][-1].delete_btn)
                self.info[pn]['del_btns'][-1].clicked.connect(
                    lambda:
                    self.del_line(pn, len(self.info[pn]['del_btns']) - 1))
                ...
                if self.info[pn]['ans_type'] == 0:
                    self.info[pn]['ans_group'].addButton(
                        self.info[pn]['ans_lines'][-1].radioButton)

                    if len(self.info[pn]['ans_lines']) == 1:
                        self.info[pn]['ans_lines'][0].radioButton.setChecked(
                            True
                        )

            else:

                self.info[pn]['ans_lines'][0].setPlaceholderText(
    ...
    def run(self):
        if not self.fileName:
            a = QMB.warning(self, "Запуск файла", "Необходимо сохранить файл")
            print(a)
            self.saveAs()
        else:
            self.save()
    ...
    def open(self):
        # Открытие файла
        fileName = QFD.getOpenFileName(self, "Открыть файл", "",
                                       'UnderTest files (*.*)')[0]
        if fileName:
    ...
    def save(self):
        ...
        try:
            for pn in range(len(self.info)):
                self.data[pn]['question'] = self.info[pn][
                    'page'
                ...
                if self.data[pn]['type'] == 0:
                    ...
                    for j in range(len(btns)):
                        self.data[pn]['vars'].append(self.info[pn][
                            'ans_lines'
                        ][j].ans.text())
                        if btns[j].isChecked():
                    ...
                    self.data[pn]['vars'] = ['']
                    self.data[pn]['rightAnswers'] = self.info[pn][
                        'ans_lines'
                    ][0].text()
    ...
    def saveAs(self):
        fileName = QFD.getSaveFileName(self, "Сохранить файл", "",
                                       'UnderTest files (*.UT)')[0]
        if fileName:
            self.fileName = fileName
            spl = fileName.split('.')
            if spl[-1] != 'UT':
...
class ShowTest(QMainWindow):
    ...
    def __init__(self, fileName=None):
        ...
        while not fileName:
            fileName = QFD.getOpenFileName(self, "Открыть файл", "",
                                           'UnderTest files (*.UT)')[0]
            if not fileName:
                reply = QMB.question(self, "Выберите файл",
                                     "Вы не выбрали файл",
                                     QMB.Ok | QMB.Cancel, QMB.Cancel)
                print(reply)
                if reply == 0 or reply == QMB.Cancel:
                    break
        if fileName:
            ...
            if self.data[i]['type'] == 0:
                ...
                for j in range(n):
                    ...
                    if j == n - 1 and j % 2 == 0:
                        ...
                        if j % 2 == 0:
                            self.info[i]['page'].verticalLayout_2.addWidget(
                                chb
                                )
                        else:
                            self.info[i]['page'].verticalLayout_3.addWidget(
                                chb
                                )
            else:
                le = QLineEdit()
        ...
        try:
            ...
            for i in range(len(self.info)):
                if self.data[i]['type'] == 0:
                    ...
                    if self.info[i]['btns'].text() == self.data[i][
                        'rightAnswers'
                    ]:
                        self.good_ans += 1
                    else:
    ...
    def draw_pie(self):
        ...
        try:
            ...
            plt.pie(good_bad, startangle=90.0, autopct=shownum,
                    colors=['green', 'red'])
            plt.legend(bbox_to_anchor=(-0.16, 0.45, 0.25, 0.25),
                       loc='lower left', labels=data_names)
            self.fileName = '.'.join(
                self.fileName.split('/')[-1].split('.')[:-1])
